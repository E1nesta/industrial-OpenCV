...
`industrial-OpenCV` 是一个基于 Qt + OpenCV 的工业机器视觉上位机项目，用于组织输入源接入、预览取帧、图像巡检、结果展示、记录留痕与结果输出等完整桌面端视觉业务流程。
...
本项目重点不在单一算法样例，而在于构建一套清晰、可维护、可继续扩展的 Qt/C++ 工程骨架。
...
`输入源 -> 预览/取帧 -> 巡检任务创建 -> 图像处理 -> 结果判定 -> UI 展示 -> 图片归档 -> 记录落库 -> TCP 输出`
...
- `InspectionResult`
  表示一次巡检的最终结果，包含 OK/NG、缺陷数量、耗时等摘要信息。

- `InspectionOutput`
  表示巡检完成后的统一出口对象，用于 UI 展示、持久化和 TCP 输出。

- `Recipe`
  表示巡检参数配置，例如阈值、面积范围、ROI、形态学开关、图片保存路径等。
