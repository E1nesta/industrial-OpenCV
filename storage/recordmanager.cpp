...

    {
        ...
        if (database.isOpen()) {
            ...
            query.prepare(QStringLiteral(
                ...
                "(inspection_id, timestamp, batch_no, is_ok, defect_count, process_time_ms, image_path, result_image_path) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?)"));
    ...
    QString *errorMessage) const
{
...

    {
        ...
        if (database.isOpen()) {
            QSqlQuery query(database);
            query.prepare(QStringLiteral(
                "SELECT inspection_id, timestamp, batch_no, is_ok, defect_count, process_time_ms, image_path, result_image_path "
            ...
            if (query.exec()) {
                if (query.next()) {
                    ...
                    record->batchNo = query.value(2).toString();
                    record->isOk = query.value(3).toInt() != 0;
                    record->defectCount = query.value(4).toInt();
                    record->processTimeMs = query.value(5).toDouble();
                    record->imagePath = query.value(6).toString();
                    record->resultImagePath = query.value(7).toString();
                    found = true;
                }
